    ...
    <div className="space-y-12">
      <div className="animate-fade-in-up space-y-4 pt-8 text-center">
        ...
        <div className="flex items-center justify-center gap-3 pt-2">
          <Link
            href="/signup"
            className="gradient-button rounded-lg px-6 py-2.5 text-sm font-semibold text-white"
          >
            Create your store
          </Link>
          <Link
            href="/admin/login"
            className="glass-card rounded-lg px-6 py-2.5 text-sm text-white/70 transition-colors hover:text-white"
          >
            Sign in
          </Link>
        </div>
      ...
      <div className="animate-fade-in-up space-y-4">
        <h2 className="text-lg font-semibold text-white/80">Browse stores</h2>
        {organizations.length === 0 ? (
          <p className="text-white/50">No stores yet — be the first.</p>
        ) : (
            ...
            {organizations.map((organization, index) => (
              ...
              >
                <Link href={`/store/${organization.slug}`} className="block p-6">
                  <p className="text-lg font-semibold text-white transition-colors hover:text-pink-300">
                    {organization.name}
                  </p>
